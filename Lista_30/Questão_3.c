...
void backupDados(FILE *cadastrosN, FILE *cadastrosI, FILE *tamanho, Pessoa pessoas[])
{
    ...
    if (cadastrosI != NULL)
    {
        for (int i = 0; i < qtPessoas; i++)
        {
            fscanf(cadastrosI, "%d/%d/%d", &pessoas[i].dia, &pessoas[i].mes, &pessoas[i].ano);
        }
    }
    fclose(cadastrosI);
...
{
    printf("Nome: %s, data de aniversário: %d/%d/%d\n\n", pessoas[i].nome, pessoas[i].dia, pessoas[i].mes, pessoas[i].ano);
}
...
void cadastraIdades(Pessoa pessoas[], FILE *cadastro)
{
    ...
    scanf("%d/%d/%d", &pessoas[TAM].dia, &pessoas[TAM].mes, &pessoas[TAM].ano);
    cadastro = fopen("cadastroIdade.dat", "a");
    fprintf(cadastro, "%d/%d/%d\n", pessoas[TAM].dia, pessoas[TAM].mes, pessoas[TAM].ano);
    fclose(cadastro);
}
